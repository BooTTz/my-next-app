...
**标签**: `v1.2.2`  
**对应提交**: `84ff726`  
**GitHub**: https://github.com/BooTTz/my-next-app/releases/tag/v1.2.2

### 包含内容
- **工作组管理页面重构**：监管部门/服务机构/企业单位三大组织标签页全面重构，新增左侧行政区划树区域筛选功能，右侧组织列表增加搜索过滤
- **组织卡片优化**：组织展示项新增角色标签、成员数量统计，调整头像尺寸和信息密度，服务机构和企业的详情信息（资质/主营业务、行业/规模）优化展示
- **侧边栏导航重组**：工作台作为独立入口，"检查"相关模块（检查事项/隐患管理/检查报告/统计分析）合并为可折叠分组，支持展开/折叠交互
- **面包屑层级优化**：检查相关模块面包屑统一添加"检查"前缀，新增组织详情面包屑路径
- **返回链接修复**：组织详情页返回按钮指向 `/workspace/settings`
- **工作台布局集成面包屑**：工作台顶部区域改用 `PageBreadcrumb` 组件

### 改动文件
- `src/app/workspace/settings/page.tsx` - 组织管理三大标签页重构（行政区划树+搜索+卡片优化）
- `src/components/layout/AppSidebar.tsx` - 侧边栏导航重组（工作台独立+检查折叠组）
- `src/lib/breadcrumb-config.ts` - 面包屑配置更新（检查前缀+组织详情路径）
- `src/app/workspace/layout.tsx` - 工作台顶部集成面包屑组件
- `src/app/team/[teamId]/page.tsx` - 返回链接修复

### 如何恢复到此版本
```bash
# 方式1：创建新分支恢复（推荐）
git checkout -b restore-v1.2.2 v1.2.2
